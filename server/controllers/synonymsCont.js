...
exports.createSynonyms = async (req, res) => {
    try {
        ...
        if (!parentId || !childId) {
            return res.status(400).json({ error: 'Both subcategory IDs are required.' });
        }

        if (!mongoose.isValidObjectId(parentId) || !mongoose.isValidObjectId(childId)) {
            return res.status(400).json({ error: 'Invalid MongoDB ID provided for one or both subcategories.' });
        }
        ...
        if (!parentSubcategory || !childSubcategory) {
            return res.status(404).json({ error: 'Either parent or child subcategory not found.' });
        }
        ...
        await SubCategory.findByIdAndUpdate(childSubcategory._id, { disable: true });

        return res.status(200).json({
            success: true,
            message: "Synonym created, events updated, and child subcategory disabled successfully.",
            newSynonym,
...
exports.editSynonym = async (req, res) => {
    try {
        ...
        if (!synonymId || !newChildId) {
            return res.status(400).json({ error: 'Synonym ID and new child subcategory ID are required.' });
        }

        if (!mongoose.isValidObjectId(synonymId) || !mongoose.isValidObjectId(newChildId)) {
            return res.status(400).json({ error: 'Invalid MongoDB ID provided for synonym or new child subcategory.' });
        }

        const synonym = await Synonyms.findById(synonymId).populate('childSubCategory');
        if (!synonym) {
            return res.status(404).json({ error: 'Synonym not found.' });
        }

        // Check if childId has actually changed
        if (synonym.childSubCategory && synonym.childSubCategory._id.toString() === newChildId) {
            return res.status(400).json({ message: 'New child ID is the same as the current one.' });
        }
        ...
        const [currentChildSubcategory, newChildSubcategory] = await Promise.all([
            SubCategory.findById(synonym.childSubCategory ? synonym.childSubCategory._id : null),
            SubCategory.findById(newChildId)
        ]);

        if (!newChildSubcategory) {
            return res.status(404).json({ error: 'New child subcategory not found.' });
        }

        // Update the old child subcategory to re-enable it
        if (currentChildSubcategory) {
            await SubCategory.findByIdAndUpdate(currentChildSubcategory._id, { disable: false });

            // Update events linked to the old child subcategory to revert changes
            await Event.updateMany(
                { subCategory: currentChildSubcategory._id },
                { $set: { subCategory: synonym.subCategory, category: synonym.subCategory.category }}
            );
        }
...
exports.getSynonymById = async (req, res) => {
    try {
        ...
        const synonym = await Synonyms.findById(id).populate('subCategory').populate('childSubCategory');
...
exports.getSynonymsByParentId = async (req, res) => {
    try {
        ...
        const synonyms = await Synonyms.find({ subCategory: parentId }).populate('subCategory').populate('childSubCategory');
